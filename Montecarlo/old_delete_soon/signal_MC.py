...
_description = "MonteCarlo Simulation of signal from NaI crystal."
...
def main():
    ...
    E_tot = np.concatenate([arr for _list in (E_ph, E_e) for arr in _list])
    E_sgn = np.concatenate([arr for arr in E_ph])
    E_bkg = np.concatenate([arr for arr in E_e])
